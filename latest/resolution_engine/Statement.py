...
class Statement():
    ...
    def __init__(self, statement_string=None):
        if statement_string:
            ...
            statement_string_list = list(map(lambda x: x.predicate_string, self.predicate_set))
    ...
    def init_from_predicate_set(self, predicate_set):
        ...
        statement_string_list = list(map(lambda x: x.predicate_string, predicate_set))
        self.statement_string = '|'.join(statement_string_list)
    ...
    def __hash__(self):
        return hash((''.join(sorted(self.statement_string)))) # type: ignore
    ...
    def add_statement_to_KB(self, KB, KB_HASH):
        ...
        """
        KB.add(self)
        for predicate in self.predicate_set: # type: ignore
    ...
    def resolve(self, statement):
        ...
        for predicate_1 in self.predicate_set: # type: ignore
            for predicate_2 in statement.predicate_set:
                ...
                if unification == False:
                    ...
                    rest_statement_1 = list(filter(lambda x: False if x == predicate_1 else True, rest_statement_1))  # type: ignore
                    ...
                    if not rest_statement_1 and not rest_statement_2:           # contradiction found
                        return False
                    rest_statement_1 = list(map(lambda x: x.substitute(unification), rest_statement_1)) # type: ignore
                    rest_statement_2 = list(map(lambda x: x.substitute(unification), rest_statement_2)) # type: ignore
