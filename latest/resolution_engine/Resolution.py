...
def replace_predicate_by_constant(statement):
    ...
    """
    r = re.compile('~?[A-Z][A-Za-z0-9]*\([a-zA-Z0-9][a-zA-Z0-9,]*\)')
    predicates = r.findall(statement)
    predicates_dict = {}
...
def get_cnf_statements(fol_sentence):
    """
    Converts a single FOL sentence string to a list of CNF Statement objects.
    """
    global predicates_dict
    predicates_dict.clear()
    statement, predicates_dict = replace_predicate_by_constant(fol_sentence)
...
def prepare_knowledgebase(FOL_SENTENCES):
    ...
    for statement in FOL_SENTENCES:
        cnf_objects = get_cnf_statements(statement)
        for stmt_obj in cnf_objects:
...
def FOL_Resolution(KB, KB_HASH, query_clauses):
    """
...
def run_resolution_engine(input_file='input.txt', output_file='output.txt'):
    global INPUT_FILE, OUTPUT_FILE, KNOWLEDGE_BASE, KNOWLEDGE_BASE_HASH, STANDARD_VARIABLE_COUNT
    ...
    for query_str in QUERIES:
        # Negate the query
        negated_query = f"~({query_str})"
        
        # Convert to CNF clauses
        query_clauses = get_cnf_statements(negated_query)
        
        KB = copy.deepcopy(KNOWLEDGE_BASE)
        KB_HASH = copy.deepcopy(KNOWLEDGE_BASE_HASH)
        satisfiability = FOL_Resolution(KB, KB_HASH, query_clauses)
        write_output(satisfiability)
        results.append(satisfiability)
    return results

if __name__ == "__main__":
    run_resolution_engine()