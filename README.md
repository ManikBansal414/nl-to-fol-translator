...
## 1. Folder Structure
```
common/            # Shared NL→FOL translator engine
...
2. Rule matcher (`rules.yaml`) captures semantic fragments (quantifiers, predicates, implication patterns).
...
## 6. Adding New Rules
Edit `rules.yaml` to introduce new pattern mappings. After changes, re-run sentences to see updated translations. Keep patterns conservative to avoid over-matching.
...
- Unexpected translation: Inspect intermediate tokens by temporarily adding print statements in `common/matcher.py`.
