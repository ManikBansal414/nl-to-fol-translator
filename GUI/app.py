    ...
    if request.method == 'POST':
        ...
        if action == 'validate':
            ...
            if sentence:
                ...
                if fol.startswith('[ERROR]'):
                    context['kb_data'] = {
                        'action': 'add', 'sentence': sentence, 'fol': None,
                        'ok': False, 'error': fol, 'trace': []
                    }
                else:
                    ok = kb().add(fol)
                    stored_other = fol in kb().list_other()
                    context['kb_data'] = {
                        'action': 'add', 'sentence': sentence, 'fol': fol,
                        'ok': bool(ok),
                        'stored_other': stored_other,
                        'error': None if ok else 'Unrecognized formula', 'trace': []
                    }
            else:
                context['kb_data'] = {
                    'action': 'add', 'sentence': '', 'fol': None,
                    'ok': False, 'error': 'Please enter a sentence.', 'trace': []
                }
            ...
            if sentence:
                ...
                if fol.startswith('[ERROR]'):
                    context['kb_data'] = {
                        'action': 'query', 'sentence': sentence, 'fol': None,
                        'ok': False, 'error': fol, 'trace': []
                    }
                else:
                    ...
                    context['kb_data'] = {
                        'action': 'query', 'sentence': sentence, 'fol': fol,
                        'mode': kb_mode,
                        'ok_forward': bool(ok_f), 'trace_forward': trace_f,
                        'ok_backward': bool(ok_b), 'trace_backward': trace_b,
                    }
            else:
                context['kb_data'] = {
                    'action': 'query', 'sentence': '', 'fol': None,
                    'ok': False, 'error': 'Please enter a sentence.', 'trace': []
                }
        elif action == 'kb_clear':
            kb().clear()
            context['kb_data'] = {
                'action': 'clear', 'sentence': '', 'fol': None,
                'ok': True, 'error': None, 'trace': ['KB cleared']
            }
